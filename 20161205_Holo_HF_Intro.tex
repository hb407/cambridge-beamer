...
1) Consider modelling protein structure\\
2) Molecular dynamics\\
3) Need parameters!\\
4) Find parameters from wavefunction... so we need a wave function!
\end{frame}
...
\begin{frame}{Non--orthogonal Configuration Interaction}
 ...
 \begin{itemize}
  ...
  \item<3-> SCF states already scale with system size.
  \item<4-> SCF states are not mutually orthogonal, $\brket{\up{x}\Psi}{\up{y}\Psi} \neq \delta_{x y}.$
  \end{itemize}
   \vspace{1em}
 \uncover<5->{We must use Non-orthogonal Configuration Interaction.\\ 
 \begin{itemize}
